...
void loop() {
  ...
  switch (deviceMode) {
    ...
    case MODE_PAIRING:
      ...
      // Выводим экран сопряжения с принудительным обновлением каждый раз
      displayPairingScreen();

      // Обновленная логика проверки WiFi - попытка восстановления соединения
      // если WiFi не подключен, периодически проверяем статус и пытаемся подключиться
      if (!wifiConnected) {
        static unsigned long lastWifiCheckTime = 0;
        if (millis() - lastWifiCheckTime > 2000) { // Каждые 2 секунды
          lastWifiCheckTime = millis();

          if (WiFi.status() == WL_CONNECTED) {
            wifiConnected = true;
            Serial.println("WiFi подключен в режиме сопряжения!");
            // Звуковой сигнал о подключении
            tone(BUZZER_PIN, 2000, 50);
            delay(50);
            tone(BUZZER_PIN, 2500, 50);
          } else if (millis() - pairingStartTime > 15000) {
            // Если прошло более 15 секунд с начала сопряжения и WiFi не подключен,
            // пробуем переподключиться
            Serial.println("Повторная попытка подключения к WiFi...");
            WiFi.disconnect();
            delay(100);
      ...
      // Периодически проверяем статус сопряжения, но только если есть подключение к WiFi
      static unsigned long lastPairingCheckTime = 0;
      if (wifiConnected && millis() - lastPairingCheckTime > 5000) {
        checkPairingStatus();
        lastPairingCheckTime = millis();

        // Принудительно обновляем экран после проверки статуса сопряжения
        displayPairingScreen();
      }
...
String generatePairingCode() {
  ...
  // Используем комбинацию нескольких источников энтропии
  long seed = millis();
  seed = seed ^ (long)ESP.getFreeHeap(); // Добавляем данные о свободной памяти
  seed = seed ^ (long)hallPulseCount;    // Добавляем физические данные с датчика Холла
  randomSeed(seed);

  // Генерируем случайный 6-значный код
  String code = "";

  // Для надежности код должен быть строго 6 символов
  while (code.length() < 6) {
    code = ""; // Начинаем заново, если были проблемы
  ...
  // Проверяем, что код действительно сгенерирован
  if (code.length() != 6) {
    // Если что-то пошло не так, используем код по умолчанию
    code = "123456";

    // Звуковой сигнал ошибки
    tone(BUZZER_PIN, 500, 200);

    Serial.println("ОШИБКА: не удалось сгенерировать код сопряжения. Использую код по умолчанию!");
  }
...
bool startPairingMode() {
  // Сначала генерируем код, чтобы пользователь сразу его увидел
  // Код может уже быть создан в вызывающей функции, проверяем его
  if (pairingCode.length() != 6) {
    // Код не был создан или создан некорректно, генерируем новый
    pairingCode = generatePairingCode();

    // Дополнительная проверка, что код действительно имеет нужную длину
    if (pairingCode.length() != 6) {
      pairingCode = "123456"; // Резервный код, если генерация не удалась
      Serial.println("Использован резервный код после повторной попытки генерации");
    }
  }

  // Устанавливаем флаги и время запуска режима сопряжения
  pairingMode = true;
  pairingStartTime = millis();

  // Выводим код в консоль для отладки ДВАЖДЫ, чтобы точно увидеть в логах
  Serial.println("===============================");
  Serial.print("Код сопряжения: ");
  Serial.println(pairingCode);
  Serial.println("===============================");

  // Звуковая индикация - 3 коротких сигнала
  tone(BUZZER_PIN, 2000, 50);
  delay(100);
  tone(BUZZER_PIN, 2000, 50);
  delay(100);
  ...
  // Мигаем яркими цветами для привлечения внимания
  for (int i = 0; i < 3; i++) {
    setRGBColor(0, 0, 255); // Яркий синий
    delay(100);
    setRGBColor(0, 0, 0);
    delay(100);
  }

  // Принудительно сразу рисуем экран с кодом
  u8g2.clearBuffer(); // Очищаем буфер перед рисованием
  displayPairingScreen();

  // Еще раз принудительно отображаем код (дополнительная перестраховка)
  delay(100);
  displayPairingScreen();

  // Пытаемся подключиться к WiFi в фоновом режиме
  if (!wifiConnected) {
    // Отключаем WiFi, если была предыдущая попытка подключения
    WiFi.disconnect();
    delay(100);

    // Запускаем попытку подключения, но не блокируем работу
    WiFi.begin(configWifiSSID.c_str(), configWifiPassword.c_str());
    Serial.println("Подключение к WiFi в фоновом режиме...");
  }
...
void displayPairingScreen() {
  ...
  // Анимация RGB LED для привлечения внимания
  ...
  if (millis() - lastAnimTime > 400) {  // Animation speed
    ...
    // Более яркие цвета для анимации
    switch (animState) {
      case 0:
  ...
  // Заголовок - уменьшаем шрифт чтобы освободить место
  u8g2.setFont(u8g2_font_profont12_tr); // Меньший шрифт для заголовка
  char titleStr[] = "PAIRING CODE";
  int titleWidth = u8g2.getStrWidth(titleStr);
  u8g2.drawStr((128-titleWidth)/2, 12, titleStr);

  // Разделительная линия - немного выше
  u8g2.drawHLine(0, 15, 128);

  // Увеличиваем контраст - заполняем прямоугольник вокруг кода
  u8g2.drawBox(10, 22, 108, 20);

  // Отображаем код большими цифрами на инвертированном фоне
  if (pairingCode.length() > 0) {
    // Используем средний шрифт для лучшей читаемости
    u8g2.setFont(u8g2_font_profont17_tr);
    // Инвертируем цвета для лучшей видимости (белый текст на чёрном фоне)
    u8g2.setDrawColor(0);
    int codeWidth = u8g2.getStrWidth(pairingCode.c_str());
    u8g2.drawStr((128-codeWidth)/2, 38, pairingCode.c_str());
    u8g2.setDrawColor(1); // Возвращаем обычный режим рисования
  } else {
    u8g2.setFont(u8g2_font_profont12_tr);
    char genStr[] = "Generating...";
    u8g2.setDrawColor(0); // Инвертируем цвета
